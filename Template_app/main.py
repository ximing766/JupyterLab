...
from components.constants import (
    WINDOW_TITLE, INITIAL_GEOMETRY_X, INITIAL_GEOMETRY_Y, INITIAL_GEOMETRY_WIDTH, INITIAL_GEOMETRY_HEIGHT,
    ...
    SPLITTER_INITIAL_SIZES, PIC_FOLDER_NAME, DEFAULT_IMAGE_FILES
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.current_theme = ThemeManager.DARK_THEME  # 默认主题
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.MouseButton.XButton2:  # Back button
            new_idx = (idx - 1 + count) % count # Ensure positive index
            self.stacked_widget.setCurrentIndex(new_idx)
            self.nav_list.setCurrentRow(new_idx)
        elif event.button() == Qt.MouseButton.XButton1:  # Forward button
            new_idx = (idx + 1) % count
            self.stacked_widget.setCurrentIndex(new_idx)
            self.nav_list.setCurrentRow(new_idx)
        else:
