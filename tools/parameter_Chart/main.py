            ...
            c = (
                Line(
                    init_opts=opts.InitOpts(
                        ...
                        animation_opts=opts.AnimationOpts(
                            animation=self.style_config["animation"]
                        ),
                    )
                )
                ...
                .set_global_opts(
                    ...
                    toolbox_opts=(
                        opts.ToolboxOpts(
                            ...
                            feature=opts.ToolBoxFeatureOpts(
                                save_as_image=opts.ToolBoxFeatureSaveAsImageOpts(
                                    background_color=self.style_config[
                                        "background_color"
                                    ],
                                    pixel_ratio=3,
                                )
                        ...
                        if self.style_config["show_toolbox"]
                        else None
                    ),
                    visualmap_opts=(
                        opts.VisualMapOpts(is_show=self.style_config["show_visualmap"])
                        if self.style_config["show_visualmap"]
                        else None
                    ),
...
class StyleConfigDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.area_opacity_slider.setValue(
            int(self.config.get("area_opacity", 0.3) * 100)
        )
...
class ParameterChartApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.info_label.setStyleSheet(
            "color: #6c757d; font-style: italic; font-size: 11px;"
        )
        ...
        self.chart_type_combo.addItems(
            ["Line Chart", "Bar Chart", "Scatter Chart", "Box Chart"]
        )
    ...
    def generate_charts(self):
        ...
        selected_column_indices = sorted(
            list(set(index.column() for index in self.table_view.selectedIndexes()))
        )
        ...
        if not selected_column_indices:
            QMessageBox.warning(
                self,
                "Warning",
                "Please select at least one column in the data preview table for visualization",
            )
            return
